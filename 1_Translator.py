...
search_pharaoh = st.text_input("🔍 Search for a Pharaoh (e.g., Tutankhamun, Ramses II):").strip().lower()
...
uploaded_file = st.file_uploader("Upload a hieroglyph image", type=["jpg", "jpeg", "png"])
...
if uploaded_file is not None:
    ...
    code, name, desc, confidence = predict_image("temp.jpg")
    st.markdown(f"### 🔮 Prediction: **{name}** ({code})")
    st.markdown(f"**Confidence:** {confidence:.2f}")

    if st.button("📖 Show Meaning", key="uploaded"):
        st.info(desc)
...
search = st.text_input("🔍 Search for a hieroglyph (e.g., Ankh, Ra, Scarab):").strip().lower()
...
filtered_gallery = {k: v for k, v in gallery.items() if search in k.lower()} if search else gallery
...
for idx, (name, (path, desc)) in enumerate(filtered_gallery.items()):
    with cols[idx % 3]:
        ...
        if st.button(f"📖 Show Meaning for {name}", key=idx):
            st.info(desc)
